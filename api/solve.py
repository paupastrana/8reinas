...
class handler(BaseHTTPRequestHandler):
    def _send_json(self, obj, status=200):
        ...
        # CORS (por si algún día lo necesitas desde otro dominio)
    ...
    def do_POST(self):
        ...
        try:
            ...
            if not (4 <= N <= 12):
                self._send_json(
                    {"error": f"N debe estar entre 4 y 12. Se recibió N={N}"}, status=400
                )
                return
            ...
            if success:
                self._send_json({"steps": algorithm_steps})
            else:
                self._send_json(
                    {"steps": [], "message": f"No solution found for N={N}"}
                )

        except Exception as e:
            self._send_json({"error": f"Error interno: {str(e)}"}, status=500)
